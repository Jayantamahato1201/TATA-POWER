export type UserRole = 'ADMIN' | 'STAFF' | 'VIEWER' | 'OPERATOR' | 'ENGINEER';
...
export interface ColumnMetadata {
  ...
  type?: 'numeric' | 'string' | 'datetime' | 'category' | 'boolean' | string;
  unit?: string;
  min?: number;
  max?: number;
  avg?: number;
...
export interface AlarmAudioSettings {
  ...
  repeatIntervalSec: number; // 0 for once, or 10, 15, 30, 60
